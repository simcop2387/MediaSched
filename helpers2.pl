...
sub getDirectoryList
{
  ...
  for (@prelist)
  {
    push @list, $parent."/".$_ if (-f "$parent/$_" && /(flv|wmv|asf|rm(vb)?|ogm|mkv|avi|mpe?g)$/i);
  }
...
for (1..100)
{
  my $l="!TV Shows/Futurama:7\n!simpsons:20\n!TV Shows/Family Guy:11\n!TV Shows/Robot Chicken:5";
  my $cid=666;

  print parseListEntry($l,$cid)."\n";

}

#for my $dir (map {$videopath."/".$_} ("TV Shows/Futurama", "simpsons", "TV Shows/Family Guy", "TV Shows/Robot Chicken"))
#{
# my $count = scalar getFileList($dir);
# print "$dir  ::  $count \n";
#}
